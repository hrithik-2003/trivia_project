...
class FakeYou2(Service):
    ...
    def tts_poll(self, ijt: str):
        while True:
            ...
            wav = Wav(ijt_data)
            if wav.status in ["started", "pending"]:
                continue
            elif wav.status == "attempt_failed":
                return Failed()
            elif wav.status == "dead":
                return Dead()
            elif wav.status == "complete_success":
    ...
    def get_wav_content(self, wav, ijt_data):
        if wav.link:
            # If link starts with http or https, use it directly.
            print(wav.link)
            print(self.api(wav.link))
            url = wav.link
            print(url)
